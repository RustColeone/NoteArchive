...
{
   float ans = sqrt(pow((p.x - q.x),2) + pow((p.y - q.y),2));
   return ans; // @@@ STUB
...
void initPoint(struct Point *p, double xVal, double yVal)
{
   ...
   //It follows the reference and changed the referenced data
}
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance)
{
   ...
   Point p1 = b1.ul;
   Point p2 = b2.ul;
   Point p01 = {b1.ul.x + b1.width, b1.ul.y - b1.height};
   Point p02 = {b2.ul.x + b2.width, b2.ul.y - b2.height};

   return (pointsApproxEqual(p1,p2) && pointsApproxEqual(p01,p02)); // STUB!  TODO: Delete this line and comment and replace with appropriate code
}
...
{  
   return abs(b.width)*abs(b.height); /* stub---make sure all tests fail initially */
